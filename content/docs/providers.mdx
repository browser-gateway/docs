...
Take a provider out of rotation without deleting it. Its URL, key and settings stay in `gateway.yml`, so switching it back on restores it exactly as it was.

When to use this: a provider is having an outage, you are over its monthly quota, or you want to test the rest of your setup without it.
...
While a provider is off, the gateway behaves as if it was never added:

- New sessions never go to it, whatever the routing strategy.
- A request that names it with `?provider=primary` gets the same `400` as a provider that does not exist.
- It is not health-checked and does not appear in `/v1/status`, the playground or the API page.
- Sessions that were already running on it finish normally.

It still appears in `GET /v1/providers` with `enabled: false`, and on the dashboard Providers page with a switch to turn it back on. From the API:

```bash
curl -X PUT http://localhost:9500/v1/providers/primary \
  -H "Authorization: Bearer $BG_TOKEN" \
  -H "Content-Type: application/json" \
  -d '{"enabled": true}'
```

Omitting `enabled` means the provider is on.
...
## How Provider Credentials Are Shown Back To You

Provider URLs and headers usually carry a key of some kind. When you list providers
over the API or view them in the dashboard, those values come back masked as `***`,
so nothing that can open a browser session is echoed out of the gateway. What gets
masked:

- Query parameters in a provider URL whose name looks like a credential: `token`,
  `apiKey`, `api_key`, `key`, `secret`, `password`, `access_token`, `jwt`, `sig` and
  similar.
- A username or password written into the URL itself, as in `wss://user:pass@host`.
- The value of any header whose name carries a credential, such as `Authorization`,
  `X-Api-Key` or `Cookie`.
...
```json
{
  "providers": [
    {
      "id": "cloud-primary",
      "url": "wss://provider.example.com/?token=***",
      "headers": { "Authorization": "***", "X-Client": "browser-gateway" },
      "maxConcurrent": 5,
      "priority": 1
    }
  ]
...
The real values stay in `gateway.yml` and are still sent to the provider. If you send
a masked value straight back in a `PUT /v1/providers/:id`, the gateway keeps what it
already had rather than storing `***`, so editing one field of a provider in the
dashboard does not wipe the others. To change a credential, send the new value.
