...
| `BG_ALLOWED_HOSTS` | Comma-separated list of extra `Host` header values accepted on `/mcp` when `BG_TOKEN` is unset. Default: loopback only. |
| `PORT` | Server port. Default 9500. 12-factor convention used by Railway, Render, Fly, Heroku. |
| `HOST` | Bind interface. The image sets this to `0.0.0.0`, so a container listens on every interface whether or not `BG_TOKEN` is set. |
