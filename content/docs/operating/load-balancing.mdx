...
If the strategy tries every provider and they're ALL full or in cooldown, the connection enters the [request queue](/operating/queue) (if enabled). The client waits until a slot opens up on any provider.

See [Request Queue](/operating/queue) for details.
