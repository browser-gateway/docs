...
- **Dashboard** at `/web/webhooks`, add, edit, test, and delete webhooks with a form. Preferred for day-to-day changes because updates take effect without a restart.
- **REST API** under `/v1/webhooks`, same operations for scripted setups. See the [Webhooks REST reference](/rest-api/webhooks).

For static configs, a `webhooks` section in `gateway.yml` still works:
...
Every webhook has a per-webhook secret used to sign the request body with HMAC-SHA256. The signature is sent as an `X-BG-Signature` header:

```
X-BG-Signature: sha256=<hex-digest>
```
...
The secret is shown once when a webhook is created and can be rotated from the dashboard. Rotating invalidates the previous signature.
