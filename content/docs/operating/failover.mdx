...
1. Gets the list of configured providers, ordered by your [routing strategy](/operating/load-balancing)
...
6. If all fail and the [request queue](/operating/queue) is enabled, the connection waits for a slot to open
...
Cooldown applies the same 50% failure rate / 3 requests rule regardless of how many providers are configured. If you have only one provider and it fails 3+ times in a 60-second window, that provider enters cooldown for 30 seconds and every new connection during that window fails with `no eligible provider available`.

If you run a single provider, either configure a fallback or raise `failureThreshold` and `minRequestVolume` so transient errors do not take your gateway offline.
...
**With queue enabled (default)**: The connection waits in the queue. As soon as any provider gets a free slot (either a cooldown expires or an existing session ends), the waiting connection is routed to it. See [Request Queue](/operating/queue) for details.
...
- **Combine failover with webhooks.** Get notified when providers go down instead of discovering it from user complaints. See [Webhooks](/operating/webhooks).
