...
## Data-plane REST timeouts

The one-shot data-plane endpoints (`POST /v1/screenshot`, `POST /v1/content`, `POST /v1/scrape` on `https://cdp.browsergateway.io`) accept a `timeout` field in the request body:

| Field | Default | Min | Max | Controls |
|---|---|---|---|---|
| `timeout` | `30000` | `1000` | `120000` | Per-navigation timeout in milliseconds. Also drives the outer request wall-clock. |
| `waitForSelector` | none | | | CSS selector to wait for after navigation. |
| `waitForTimeout` | none | `0` | `30000` | Extra milliseconds to wait after load. |
| `retries` | `2` | `0` | `5` | Retry attempts on navigation failure. |

The router derives the outer request budget as `timeout + 20000ms` (20 seconds of slack for provider connect + response serialization). Set `timeout` higher for slow external upstreams:

```bash
curl -X POST "https://cdp.browsergateway.io/v1/screenshot" \
  -H "Authorization: Bearer $BG_ROUTER_KEY" \
  -H "Content-Type: application/json" \
  -d '{"url":"https://example.com","timeout":90000}'
```

Cloudflare Browser Rendering (Kitesurf) and other beta providers can be 5-25× slower than mainstream cloud browsers on complex pages. Raise `timeout` up to the 120-second max if you see 408 responses.
...
Store `secret` immediately. It is used to verify HMAC signatures on delivery. Never returned again.
...
- `DELETE /v1/access-tokens/:id`: delete a bearer token
...
## Rotating and deleting access tokens

Rotating an access token (via the dashboard) gives the same token a new value. The row keeps its label and creation date, and the old value returns 401 immediately, so update every place that uses it right after rotating.

Revoking switches a token off but keeps it in the list. Deleting removes it from the list, whether it is active or revoked.

Router keys (`POST /v1/api-keys/rotate`) work differently: the previous key keeps working for 24 hours.
