...
- `Page.setDeviceMetricsOverride` must be sent before `Page.startScreencast` or Chromium silently emits zero frames. See [puppeteer#10527](https://github.com/puppeteer/puppeteer/issues/10527). The plugin handles this.
- The plugin lives inside the CDP byte pipe. No separate CDP socket is opened. Nothing runs inside the page being scraped. Capture is invisible to the site under test.
...
Restart the gateway. Then opt in per session by adding `?session_record=true` to the connect URL:
...
Recording is per-session opt-in. `session_record` is a URL parameter, not a config toggle, so the same gateway can record some sessions and skip others. REST one-shot endpoints are not recorded. Use the [Playground](/live-playground) for interactive inspection of a single page.
