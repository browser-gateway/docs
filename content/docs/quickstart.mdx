...
`${API_KEY}` is resolved from the environment (or a `.env` file) at startup. See [Configuration → Custom headers](/configuration#custom-headers) for header merge order and limits.
