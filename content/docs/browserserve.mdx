...
Environment variables: `PORT` (default 9222), `HOST` (default `0.0.0.0`), `BROWSERSERVE_TOKEN` (enables token auth), `BROWSERSERVE_CONFIG`, `BROWSERSERVE_CHROME_PATH`, `BROWSERSERVE_DATA_DIR`, `BROWSERSERVE_MIN_READY` (mirrors `pool.minReady`), `BROWSERSERVE_MAX_SESSIONS` (mirrors `pool.maxSessions`), `BROWSERSERVE_MAX_QUEUE` (mirrors `pool.maxQueue`), `BROWSERSERVE_QUEUE_TIMEOUT_MS` (mirrors `pool.queueTimeoutMs`), `BROWSERSERVE_IDLE_TIMEOUT_MS` (mirrors `session.idleTimeoutMs`), `BROWSERSERVE_MEMORY_MAX_MB` (mirrors `session.memoryMaxMb`), `BROWSERSERVE_NO_SANDBOX` (mirrors `chrome.noSandbox`), `BROWSERSERVE_REQUIRE_SANDBOX` (mirrors `chrome.requireSandbox`). Env vars override the YAML file. Every tuning knob has an env var equivalent, so managed-platform deploys never need a mounted YAML.
