...
- **Resource limits.** Kernel-enforced per-session limits on a delegated host: memory (`session.memoryMaxMb`), CPU (`session.cpuPercent`) and process count (`session.pidsMax`). Elsewhere only a polled RSS soft cap for memory applies. At startup browserserve prints which of the three the host can enforce.
- **Profile directory.** How each session's directory is provisioned: `reflink` (copy-on-write, on btrfs or xfs) or `plain-copy` (everywhere else). A plain copy behaves identically and is only less efficient.

The isolation contract (fresh browser, fresh profile directory, wiped on disconnect) holds on every tier.

### Per-session limits (slots)

From 0.1.11 each browser is started inside its own cgroup, so the limits cover every process of that browser: the main process, the GPU process and every tab. Together the three limits define a fixed-size slot per session. One heavy session then cannot slow its neighbours, and a host always fits the same number of sessions.

What happens when a session reaches a limit:

- **Memory.** The kernel stops the tab that used the memory. The rest of that session and all other sessions keep running.
- **CPU.** The session is slowed to its share. Other sessions are unaffected.
- **Process count.** New processes and threads are refused inside that session only.

Kernel-enforced limits need a writable, delegated cgroup v2 subtree:

- **Docker (Engine 28 or newer).** Run with `--cgroupns=private --security-opt writable-cgroups=true`. Mounting the host's `/sys/fs/cgroup` into the container does not work on most Linux hosts.
- **systemd service.** Set `Delegate=yes` on the unit.
- **Managed platforms (Cloud Run, Railway and similar).** The cgroup tree is read-only, so the portable tier applies. Size the instance instead, one session per instance if you need hard limits.

All three limits are off by default.
...
| `session.cpuPercent` | `0` | Per-session CPU cap as a percentage of one core (`100` = one core, `150` = one and a half). Kernel-enforced on a delegated host only; `0` = disabled. |
| `session.pidsMax` | `0` | Per-session cap on processes plus threads. Kernel-enforced on a delegated host only; `0` = disabled. A browser session typically uses 120 to 160. |
...
| `session.singleUse` | `false` | Serve exactly one session, then exit with code `0`. Later connections get `503 single_use_spent`. Forces `pool.maxSessions: 1` and skips calibration. For platforms that start a fresh instance when one exits (Cloud Run, Kubernetes, Fly Machines), so no two clients share a process or machine. Available from 0.1.12. |
...
| `BROWSERSERVE_CPU_PERCENT` | `session.cpuPercent` | Per-session CPU cap, percent of one core. `0` disables. |
| `BROWSERSERVE_PIDS_MAX` | `session.pidsMax` | Per-session cap on processes plus threads. `0` disables. |
| `BROWSERSERVE_SINGLE_USE` | `session.singleUse` | One session per process, then exit. Boolean. |
...
When a session is refused, the server answers 503 with a reason (`pressure`, `queue_full`, `queue_timeout`, `draining`, `launch_failed`, `calibrating`) and logs one line with the same reason. New sessions are refused above 95% CPU or memory so that running sessions are not slowed down; clients should retry or use another instance.
...
The per-session memory cap (`session.memoryMaxMb`) is disabled by default. In our own tests on 4-core, 16 GB cloud machines a session browsing text-heavy pages used about 0.5 to 0.7 GB across its whole process tree, and ad-heavy news sites used more, so a cap that seems generous can be tight in practice. A breach stops the offending tab, not the session. Recommended pattern:
