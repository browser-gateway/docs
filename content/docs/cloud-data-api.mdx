...
Every successful call draws `$0.001` from your workspace wallet. Errors (4xx and 5xx responses) never charge. Free-tier workspaces receive HTTP 402 with a link to add credits.
...
Each successful call is $0.001 (0.1 cent). Wallet balance must be above zero when the request arrives. Failure modes:

| Response | Meaning | Wallet charged? |
|---|---|---|
| 200 | Success | Yes, $0.001 |
| 400 | Validation error or provider not configured | No |
| 402 | Insufficient wallet balance | No |
...
Data API calls do **not** count against the 100 000 free BYOK sessions per month. They are a separately-priced product, similar to session recording and observability broadcast.
