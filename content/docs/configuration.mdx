...
Setting `multiProfile: true` on any other provider is **rejected at add-time**:

```
POST /v1/providers { multiProfile: true, url: "wss://external.example.com/..." }
→ HTTP 400 "multiProfile is only supported on the self-hosted runtime"
```
